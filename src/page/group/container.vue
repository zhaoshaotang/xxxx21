...
import { getObj, getObjBlock} from '@/api/visual'
...
export default {
  ...
  mounted () {
        console.log('=========2222222=============');
    // console.log(this.contain.config.scale)
    // console.log(this.stepScale)

	//获取url里的token，id
    this.setToken = getUrlParames("token")
    this.uniId = getUrlParames("id")
	//设置本地存储
    localStorage.setItem('token',this.setToken)
    localStorage.setItem('uniId',this.uniId)
  ...
  methods: {
    ...
    initData () {
      ...
      if(getUrlParames('configId')){
        // let params = {
        //   id:getUrlParames('id'),
        //   configId : getUrlParames('configId'),
        //   token : getUrlParames('token')
        // }

        axios.defaults.headers.common['Authorization'] = getUrlParames('token')
    let params =  "id=" + getUrlParames('id') + "&configId=" + getUrlParames('configId') + "&token=" + getUrlParames('token')
		axios.get(visUrl+'/panel/visual/detail?'+params).then((res) => {
          const callback = () => {
            //赋值属性
