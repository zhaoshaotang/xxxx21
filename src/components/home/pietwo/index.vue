<template>
  <div class="cus-body">
    ...
    <div class="piebody">
      <div class="jl-chart" :id="`pie-three${option.domId}`"></div>
    </div>
    <more-data
...
export default {
  ...
  methods: {
    getCharts(type) {
      let url = `/prod-api/wisdomScreen/home/wsHome/projectPersonnel`;
      axios
        .get(url, {
          params: this.params,
        })
        .then((res) => {
          ...
          if (res.data.code === 200) {
            ...
            this.addPie(
              row.management,
              row.laborSubcontract,
              row.presenceNumber
            );
          }
        });
    },
    addPie(management, laborSubcontract, presenceNumber) {
      ...
      var trafficWay = [
        {
          name: "劳务分包",
          value: parseInt(laborSubcontract),
        },
        {
          name: "管理人员",
          value: parseInt(management),
        },
        {
          name: "专业分包",
          value: parseInt(presenceNumber),
        },
      ];
      var data = [];
      var color = ["#01ffff", "#ff2261", "#ffe53d"];
      for (var i = 0; i < trafficWay.length; i++) {
        data.push(
          {
            value: trafficWay[i].value || 0,
      ...
      pieOne.on("click", (param) => {
        if (param.name === "管理人员") {
          this.params.personnelType = 1;
          this.params.trendTypeName = "管理人员信息";
        } else if (param.name === "专业分包") {
          this.params.personnelType = 2;
          this.params.trendTypeName = "专业分包人员信息";
        } else if (param.name === "劳务分包") {
          this.params.personnelType = 3;
          this.params.trendTypeName = "劳务分包人员信息";
        }
