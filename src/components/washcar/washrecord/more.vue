      ...
      >
       ...
       <el-table-column
        ...
        >
          <template slot-scope="scope">
            <el-image
              :src="scope.row.platePic"
             ></el-image>
          </template>
        </el-table-column>
        <el-table-column
          label="洗车前照片"
          prop="admissionPhotos"
          align="center"
        >
          <template slot-scope="scope">
            <el-image
...
export default {
  ...
  methods: {
    handleOpen() {
      this.queryParams.serialNumber = this.serialNumber;
      this.queryParams.result = this.result;
      this.getList();
    },
    getList() {
      let url = `/prod-api/wisdomScreen/carwash/wsCarwash/result`;
      axios
        .get(url, {
          params: {
            ...this.queryParams,
