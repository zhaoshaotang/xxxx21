...
export default {
  ...
  methods: {
    handleOpen() {
      if(this.equipmentNoType == 1){
        this.queryParams.equipmentNo = this.singleEquipmentNo;
      }else if(this.equipmentNoType == 2){
        this.queryParams.equipmentNo = this.leftEquipmentNo;
      }else{
        this.queryParams.equipmentNo = this.rightEquipmentNo;
      }
      axios.defaults.headers.common["Authorization"] = getUrlParames("token");
      this.getList()
    },
    getList() {
      let url = `/prod-api/wisdomScreen/elevator/wsElevator/violationsDetails`;
      axios
        .get(url, {
          params: {...this.queryParams, startDate: this.date[0], endDate: this.date[1]},
        })
        .then((res) => {
